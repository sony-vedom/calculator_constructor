    ...
    switch (action.type) {
        ...
        case DIVISION: {
            return {
                ...
                number: null,
                number2: null,
                result: roundingAndIsFinite(Number(state.number) / Number(state.number2 ?? "1")),
                typeOperation: "",
            }
        }
        ...
        case SET_TYPE_OPERATION: {
            return (state.result) !== null ?
                {...state, typeOperation: action.typeOperation, result: null, number: state.result ?? 0}
                : {...state, typeOperation: action.typeOperation,}
        }

        case SET_NUMBER: {
            if (!state.typeOperation) {
                const resultNumber2 = {number2: null, result: null}
                if (String(state.number + action.number).length > 14) return {
                    ...state,
                    number: state.number,
                    ...resultNumber2
            ...
            if (String(state.number2 + action.number).length > 14) return {...state, number2: state.number2}

            return (state.number2 === null || String(state.number2) === "0")
